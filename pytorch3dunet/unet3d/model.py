...
class AbstractUNet(nn.Module):
    ...
    def forward(self, x, graph_data, edge_index):
        ...
        scaled_gat_features = gat_features * 10
        expanded_gat_output_mean = scaled_gat_features.view(1, 256, 1, 1, 1).expand_as(x)
...
class GAT(nn.Module):
    def __init__(self, in_features, out_features, num_layers, heads_per_layer, dropout=0.6, concat=True):
        ...
        self.final_concat = concat   # 确定最后一层是拼接还是平均

        for i in range(num_layers):
            in_channels = in_features if i == 0 else out_features * heads_per_layer[i - 1]
            out_channels = out_features if i < num_layers - 1 or not self.final_concat else out_features * heads_per_layer[i]
            layer_concat = True if i < num_layers - 1 else self.final_concat
            self.layers.append(GATConv(in_channels, out_channels, heads=heads_per_layer[i],
                                       dropout=dropout, concat=layer_concat))
